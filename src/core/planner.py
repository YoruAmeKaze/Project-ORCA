...
3. plan 的最后一步必须是 reply。reply 的 message 必须引用上一步的 output（如 {{step.luckin_create_order.output}}），不能硬编码成功/失败消息——结果来自上一步的实际输出
4. 如果某步的结果需要被后面的步骤引用，可以用 skill 名称作为隐式 id：{{{{step.<skill名>.output}}}}。如果该步有显式 id（如 {{id: "store"}}），则用显式 id：{{{{step.store.output}}}}。注意：{{step.x.output}} 只能取完整文本输出，不支持子字段。
    当"当前已知信息"中已有明确值（如"门店ID：610287"）时，直接抄字面量（610287）填入参数，不要用 {{step.x.output}} 引用——已知信息里的值可以直接用，不需要从上一步 output 里提取
...
正确示例（点咖啡——用户指定了商品后下单）：
{"task_type": "luckin_order", "stage": "ordering", "ack": "下单了。", "reasoning": "用户选好了门店和商品，直接下单", "steps": [{"skill": "luckin_preview_order", "args": {"dept_id": 12345, "product_id": 678, "sku_code": "SKU001"}, "narration": "先预览一下。"}, {"skill": "luckin_create_order", "args": {"dept_id": 12345, "product_id": 678, "sku_code": "SKU001"}, "narration": "下单中。"}, {"skill": "reply", "args": {"message": "{{step.luckin_create_order.output}}"}}]}
